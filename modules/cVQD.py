...
class VQD:
    ...
    def __init__(self, qOps, wfnA, nEx, states=np.array([])):
        ...
        # self.optimizer = COBYLA(maxiter=1000, disp=False)
...
class LLConstrainedVQD(VQD):
    ...
    def cost_fun_gen(self, params):
        ...
        cost = np.real(ev) +  pen  #+ dnpen #cost now includes additional penalty to force total spin towards 0.
        self.densConsSat.append([self.fcount, dnpen])
        if self.fcount%50 == 0:
    ...
    def calculate(self):
        ...
        for iex in range(self.nEx+1):
            ...
            with open('densConsrSatLog_rank'+str(rank)+'.pic', 'wb') as fp:
                pickle.dump(self.densConsSat, fp)
            fp.close() 
            self.fcount = 0
            eigenval = res.fun
            eig_params = res.x
            ...
            if self.nEx >= iex:
                self.wfn_params[:,iex] = eig_params
            print(f'rank, index, eigenValue, eigenVector:{rank, iex, eigenval, eig_params}')
            self.eigenvals.append(eigenval)
        return (self.eigenvals, self.wfn_params)
